...
I worked on this project to solve a real-world supply chain problem.
...
Time features (month, seasonality)
👉 Why:
Demand depends heavily on location, product type, and seasonality
Y (Target):
Qty → number of units sold
👉 Why:
Direct representation of customer demand
### Shelf Detection
X:
Shelf images
Y:
Bounding boxes + object labels (products)
👉 Why:
Needed for detecting product presence and stock availability
